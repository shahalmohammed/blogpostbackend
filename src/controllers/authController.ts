...
/** ---- JWT helpers (typed for jsonwebtoken@9) ---- */
const JWT_SECRET: Secret = process.env.JWT_SECRET as Secret;
const JWT_EXPIRES_IN: SignOptions['expiresIn'] =
  (process.env.JWT_EXPIRES_IN as SignOptions['expiresIn']) || '7d';

const signToken = (user: { _id: mongoose.Types.ObjectId | string; role: string }) =>
  jwt.sign({ sub: String(user._id), role: user.role }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
...
/** First admin is open; then only authenticated admins can create more. Also cap to 2 admins. */
